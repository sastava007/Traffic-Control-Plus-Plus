...
class CapacityCounter(PipelineProcessor):
    ...
    def calculate_capacity(self, frame, frame_number):
        ...
        cv2.putText(base_frame, "Capacity: "+ str(capacity*100), (600,600), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (255, 255, 255), 2)
        # cv2.imshow("Mask",frbase_ame)
        # fig = plt.figure()
        # fig.suptitle("Capacity: {}%".format(capacity*100), fontsize=16)
        # plt.subplot(121),plt.imshow(base_frame),plt.title('Original')
        # plt.xticks([]), plt.yticks([])
        # plt.subplot(122),plt.imshow(t),plt.title('Capacity map')
        # plt.xticks([]), plt.yticks([])
